...
@Component({
  ...
  styleUrls: ['./utilisateur.component.scss','../../../../assets/custom-style.css']
})
export class UtilisateurComponent implements OnInit {
  ...
  getData(){
    ...
    this.spinner.show();
    this.restClient.execute('user','getByCriteria',request).subscribe(
      (res:any)=>{
  ...
  onCreateData(data?) {
   ...
   let dialogRef= this.dialogService.open(UtilisateurModalComponent, {
      context: {
        title: data,
  ...
  deleteItem(data){
    ...
    console.log('data sent to server',JSON.stringify(request));
    this.spinner.show();
    this.restClient.execute('user','delete',request).subscribe(
      (res:any)=>{
        this.spinner.hide();
