...
export class RoleModalComponent implements OnInit {
  ...
  itemToSave:any = {};
  userData:any
  constructor(protected ref: NbDialogRef<RoleModalComponent>,private dialogService: NbDialogService,
  ...
  saveItem(data){
    ...
    this.spinner.show()
    this.restClient.execute('role',data.id?'update':'create',request).subscribe(
      (res:any)=>{
        console.log('res',res);
        this.spinner.hide()
        if(res && !res.hasError){
