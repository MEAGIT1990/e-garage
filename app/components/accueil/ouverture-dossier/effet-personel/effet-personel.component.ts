...
import { EffetPersonnelModalComponent } from './effet-personnel-modal/effet-personnel-modal.component';


@Component({
  selector: 'ngx-effet-personel',
  templateUrl: './effet-personel.component.html',
  styleUrls: ['./effet-personel.component.scss','../../../../../assets/custom-style.css']
})
export class EffetPersonelComponent implements OnInit {
  ...
  getData(){
    ...
    this.spinner.show();
    this.restClient.execute('effetPerso','getByCriteria',request).subscribe(
      (res:any)=>{
  ...
  onCreateData(data?) {
   ...
   let dialogRef= this.dialogService.open(EffetPersonnelModalComponent, {
      context: {
        title: data,
  ...
  deleteItem(data){
    ...
    console.log('data sent to server',JSON.stringify(request));
    this.spinner.show();
    this.restClient.execute('effetPerso','delete',request).subscribe(
      (res:any)=>{
        this.spinner.hide();
