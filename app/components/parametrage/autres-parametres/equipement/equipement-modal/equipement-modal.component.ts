...
@Component({
  selector: 'ngx-equipement-modal',
  templateUrl: './equipement-modal.component.html',
  styleUrls: ['./equipement-modal.component.scss','../../../../../../assets/custom-style.css']
})
export class EquipementModalComponent implements OnInit {
  @Input() title: string;
  itemToSave:any = {};
  userData:any
  constructor(protected ref: NbDialogRef<EquipementModalComponent>,private dialogService: NbDialogService,
  ...
  saveItem(data){
    ...
    this.spinner.show()
    this.restClient.execute('equipement',data.id?'update':'create',request).subscribe(
      (res:any)=>{
        console.log('res',res);
        this.spinner.hide()
        if(res && !res.hasError){
