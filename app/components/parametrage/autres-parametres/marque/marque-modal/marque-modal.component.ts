...
@Component({
  selector: 'ngx-marque-modal',
  templateUrl: './marque-modal.component.html',
  styleUrls: ['./marque-modal.component.scss','../../../../../../assets/custom-style.css']
})
export class MarqueModalComponent implements OnInit {

  @Input() title: string;
  itemToSave:any = {};
  userData:any
  constructor(protected ref: NbDialogRef<MarqueModalComponent>,private dialogService: NbDialogService,
  ...
  saveItem(data){
    ...
    this.spinner.show()
    this.restClient.execute('marque',data.id?'update':'create',request).subscribe(
      (res:any)=>{
        console.log('res',res);
        this.spinner.hide()
        if(res && !res.hasError){
