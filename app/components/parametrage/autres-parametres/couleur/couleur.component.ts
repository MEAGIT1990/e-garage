...
import { CouleurModalComponent } from './couleur-modal/couleur-modal.component';
@Component({
  selector: 'ngx-couleur',
  templateUrl: './couleur.component.html',
  styleUrls: ['./couleur.component.scss','../../../../../assets/custom-style.css']
})
export class CouleurComponent implements OnInit {
  ...
  onCreateData(data?) {
   ...
   let dialogRef= this.dialogService.open(CouleurModalComponent, {
      context: {
        title: data,
  ...
  deleteItem(data){
    ...
    console.log('data sent to server',JSON.stringify(request));
    this.spinner.show();
    this.restClient.execute('couleur','delete',request).subscribe(
      (res:any)=>{
        this.spinner.hide();
