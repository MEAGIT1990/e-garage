...
import { TypeClientModalComponent } from './type-client-modal/type-client-modal.component';

@Component({
  selector: 'ngx-type-client',
  templateUrl: './type-client.component.html',
  styleUrls: ['./type-client.component.scss','../../../../../assets/custom-style.css']
})
export class TypeClientComponent implements OnInit {
  ...
  onCreateData(data?) {
   ...
   let dialogRef= this.dialogService.open(TypeClientModalComponent, {
      context: {
        title: data,
  ...
  deleteItem(data){
    ...
    console.log('data sent to server',JSON.stringify(request));
    this.spinner.show();
    this.restClient.execute('typeClient','delete',request).subscribe(
      (res:any)=>{
        this.spinner.hide();
