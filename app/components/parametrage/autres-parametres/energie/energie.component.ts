...
import { EnergieModalComponent } from './energie-modal/energie-modal.component';

@Component({
  selector: 'ngx-energie',
  templateUrl: './energie.component.html',
  styleUrls: ['./energie.component.scss','../../../../../assets/custom-style.css']
})
export class EnergieComponent implements OnInit {
  ...
  onCreateData(data?) {
   ...
   let dialogRef= this.dialogService.open(EnergieModalComponent, {
      context: {
        title: data,
  ...
  deleteItem(data){
    ...
    console.log('data sent to server',JSON.stringify(request));
    this.spinner.show();
    this.restClient.execute('energie','delete',request).subscribe(
      (res:any)=>{
        this.spinner.hide();
