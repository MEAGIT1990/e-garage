...
@Component({
  selector: 'ngx-energie-modal',
  templateUrl: './energie-modal.component.html',
  styleUrls: ['./energie-modal.component.scss','../../../../../../assets/custom-style.css']
})
export class EnergieModalComponent implements OnInit {

  @Input() title: string;
  itemToSave:any = {};
  userData:any
  constructor(protected ref: NbDialogRef<EnergieModalComponent>,private dialogService: NbDialogService,
  ...
  saveItem(data){
    ...
    this.spinner.show()
    this.restClient.execute('energie',data.id?'update':'create',request).subscribe(
      (res:any)=>{
        console.log('res',res);
        this.spinner.hide()
        if(res && !res.hasError){
