...
@Component({
  selector: 'ngx-usage-modal',
  templateUrl: './usage-modal.component.html',
  styleUrls: ['./usage-modal.component.scss','../../../../../../assets/custom-style.css']
})
export class UsageModalComponent implements OnInit {
  ...
  itemToSave:any = {};
  userData:any
  constructor(protected ref: NbDialogRef<UsageModalComponent>,private dialogService: NbDialogService,
  ...
  saveItem(data){
    ...
    this.spinner.show()
    this.restClient.execute('usage',data.id?'update':'create',request).subscribe(
      (res:any)=>{
        console.log('res',res);
        this.spinner.hide()
        if(res && !res.hasError){
