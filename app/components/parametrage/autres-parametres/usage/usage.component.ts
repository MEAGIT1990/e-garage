...
@Component({
  ...
  styleUrls: ['./usage.component.scss','../../../../../assets/custom-style.css']
})
export class UsageComponent implements OnInit {
  ...
  onCreateData(data?) {
   ...
   let dialogRef= this.dialogService.open(UsageModalComponent, {
      context: {
        title: data,
  ...
  deleteItem(data){
    ...
    console.log('data sent to server',JSON.stringify(request));
    this.spinner.show();
    this.restClient.execute('usage','delete',request).subscribe(
      (res:any)=>{
        this.spinner.hide();
