...
@Component({
  ...
  styleUrls: ['./status.component.scss','../../../../../assets/custom-style.css']
})
export class StatusComponent implements OnInit {
  ...
  onCreateData(data?) {
   ...
   let dialogRef= this.dialogService.open(StatusModalComponent, {
      context: {
        title: data,
  ...
  deleteItem(data){
    ...
    console.log('data sent to server',JSON.stringify(request));
    this.spinner.show();
    this.restClient.execute('statut','delete',request).subscribe(
      (res:any)=>{
        this.spinner.hide();
